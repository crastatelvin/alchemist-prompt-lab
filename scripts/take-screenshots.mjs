...
async function run() {
  ...
  await page.getByPlaceholder("Design your primary prompt variant...").fill(
    "Explain recursion in 3 bullet points for a beginner with one tiny example."
  );
  ...
  await page.getByPlaceholder("Design your primary prompt variant...").fill(
    "You are a great teacher. Explain recursion for a 12-year-old using bullets."
  );
  await page.getByPlaceholder("Design your challenger prompt variant...").fill(
    "Explain recursion."
  );
  await page.getByRole("button", { name: "Run Experiment" }).click();
